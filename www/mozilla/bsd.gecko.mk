...
# _GECKO_CMPAT_ALL is a list of gecko's that are compitable with another 
#  format: gecko1:gecko2
...
. endif
.endfor
# Add support for gecko other compatible geckos
.for gecko in ${_GECKO_COMPAT_ALL}
. if ${_GECKO_ALL:M${gecko:C/^[^:]+:([^:]+).*/\1/}}!=""
GOOD_USE_GECKO+=	${gecko:C/^([^:]+).*/\1/} 
. endif
.endfor
